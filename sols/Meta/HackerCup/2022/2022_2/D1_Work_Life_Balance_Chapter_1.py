...
def solve(N, M, A, queries):
    pos = [SortedList() for _ in range(4)]
    for i, a in enumerate(A): pos[a].add(i)
    
    def calc(n):
        cl, cr = [0] * 4, [0] * 4
        sl = sr = 0
        for v in range(1, 4):
            cl[v] = pos[v].bisect_right(n - 1) - pos[v].bisect_left(0)
            cr[v] = pos[v].bisect_right(N - 1) - pos[v].bisect_left(n)
            sl += v * cl[v]
            sr += v * cr[v]
        
        if sl > sr: sl, sr, cl, cr = sr, sl, cr, cl
        d = sr - sl

        res = 0 
        for a, b in [(1, 3), (1, 2), (2, 3)]:
            use = min(d // (2 * (b - a)), cl[a], cr[b])
            res += use 
            d -= use * 2 * (b - a)
            cl[a] -= use 
            cl[b] += use
