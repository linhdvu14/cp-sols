...
# https://codeforces.com/blog/entry/95975
def solve(N, A, edges):
    ...
    # mex is min unseen val
    # cnt = val cnt from root to current node
    unseen = SortedList(list(range(N+1)))
    cnt = [0] * (N+1)
    res = 0
    ...
    def dfs(u, p=-1):
        nonlocal res
        if cnt[A[u]] == 0: unseen.remove(A[u])
        cnt[A[u]] += 1
        res = max(res, unseen.val(unseen.begin()))

        for v in adj[u]:
            if v == p: continue
            yield dfs(v, u)
        
        if cnt[A[u]] == 1: unseen.add(A[u])
        cnt[A[u]] -= 1
        yield None
