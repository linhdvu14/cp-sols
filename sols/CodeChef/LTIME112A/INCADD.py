''' Increasing Addition
https://www.codechef.com/LTIME112A/problems/INCADD
'''
...
# should increase i..N-1
# each op increments a suffix of difference array

def solve(N, Q, A, queries):
    sl = SortedList()
    for i in range(1, N): sl.add(A[i] - A[i - 1])

    res = [0] * Q
    for qi, (i, x) in enumerate(queries):
        i -= 1
        if i > 0: sl.remove(A[i] - A[i - 1])
        if i < N - 1: sl.remove(A[i + 1] - A[i])
        A[i] = x
        if i > 0: sl.add(A[i] - A[i - 1])
        if i < N - 1: sl.add(A[i + 1] - A[i])
        if sl[0] < 0: res[qi] = -sl[0]
