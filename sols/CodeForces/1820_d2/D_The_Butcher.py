...
def solve(N, rects):
    S = sum(x * y for x, y in rects)
    mx = max(x for x, _ in rects)
    my = max(y for _, y in rects)

    def is_ok(reverse=False):
        sl1, sl2 = SortedList(), SortedList()
        for x, y in rects:
            sl1.add((x, y))
            sl2.add((y, x))
        if reverse: sl1, sl2 = sl2, sl1 
        ...
        while sl1:
            x1 = sl1[-1][0]
            if x1 != d1: return False
            while sl1 and sl1[-1][0] == x1:
                x, y = sl1.pop(-1)
                sl2.remove((y, x))
                d2 -= y 
    ...
    res = set()
    if is_ok(): res.add((mx, S // mx))
    if is_ok(reverse=True): res.add((S // my, my))
