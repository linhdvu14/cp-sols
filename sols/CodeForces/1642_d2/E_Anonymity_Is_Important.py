...
# maintain 
# * all cand 1 positions
# * right[l] = min r s.t. l..r has 1

# then for each op:
# 0 l r 0: remove l..r from cand set
# 0 l r 1: update right[l] = min(right[l], r)
# 1 j : 
#   * if j not in cand set -> 'NO'
#   * else, find smallest l..r containing j, l, r are cands -> 'YES' if some interval in l+1..r-1 has 1
...
def solve(N, Q, queries):
    ...
    # right[l] = min r s.t. l..r has 1
    # segtree.query(lo..hi) = min right[l] for l=lo..hi
    right = [INF] * N
    segtree = SegmentTree([INF]*N)
    
    res = []
    for ts in queries:
        if ts[0] == 0:
            l, r, x = ts[1]-1, ts[2]-1, ts[3]
            if x == 0:  # remove all cands in l..r
                it = cands._loc_left(l)
                rm = []
                while it != cands.end() and cands.val(it) <= r:
                    rm.append(cands.val(it))
                    it = cands.next(it)
                for v in rm: cands.remove(v)
            else:      # update right[l]
                right[l] = min(right[l], r)
                segtree[l] = right[l]
            ...
            if j not in cands: res.append('NO')
                ...
                it = cands._loc_left(j)
                l = cands.val(cands.prev(it)) if it != cands.begin() else -1
                r = cands.val(cands.next(it)) if cands.next(it) != cands.end() else N
