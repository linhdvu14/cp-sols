...
def solve(N, M, A, ops):
    # trains = consecutive segments of (start idx, height) where height is strictly decreasing
    segs = SortedList()
    for i, a in enumerate(A):
        if not segs or a < segs[-1][-1]:
            segs.add((i+1, a))

    res = []
    for k, d in ops:
        v = A[k-1] = A[k-1] - d
        i = segs._loc_left((k, 0))

        # case 1: k in segs[i]
        # case 2: k in segs[i-1], height(segs[i-1]) > v
        # -> update the seg containing k, subsume following segs
        if (i != segs.end() and segs.val(i)[0] == k) or (i != segs.begin() and segs.val(segs.prev(i))[1] > v):
            rm = []
            while i != segs.end() and segs.val(i)[1] >= v:
                rm.append(segs.val(i))
                i = segs.next(i)
            for a in rm: segs.remove(a)
            segs.add((k, v))          
