...
def solve(N, segs):
    res = [INF] * N
    idx = sorted(list(range(N)), key=lambda i: segs[i][1])

    # left to right
    # for each color, keep segment with rightmost right end
    ends = IntKeyDict()
    sl = SortedList()
    for i in idx:
        l, r, c = segs[i]
        for j in range(len(sl) - 1, -1, -1):
            if sl[j][1] != c:
                res[i] = min(res[i], max(l - sl[j][0], 0))
                break
        if c in ends: sl.remove((ends[c], c))
        ends[c] = max(ends.get(c, -INF), r)
        sl.add((ends[c], c))
    
    # right to left
    # for each color, keep segment with leftmost left end
    ends = IntKeyDict()
    sl = SortedList()
    for i in idx[::-1]:
        l, r, c = segs[i]
        for j in range(len(sl)):
            if sl[j][1] != c:
                res[i] = min(res[i], max(sl[j][0] - r, 0))
                break
        if c in ends: sl.remove((ends[c], c))
        ends[c] = min(ends.get(c, INF), l)
        sl.add((ends[c], c))
