...
def solve(N, Q, S, ops):
    ba, bc, ca, cb = SortedList(), SortedList(), SortedList(), SortedList()
    for i, (x, y) in enumerate(ops):
        if x == 'b' and y == 'a': ba.add(i)
        if x == 'b' and y == 'c': bc.add(i)
        if x == 'c' and y == 'a': ca.add(i)
        if x == 'c' and y == 'b': cb.add(i)
...
def main():
    ...
    for _ in range(T):
        ...
        ops = [input().decode().strip().split() for _ in range(Q)]
        res = solve(N, Q, S, ops)
        print(*res, sep='')
